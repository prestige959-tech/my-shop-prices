# index.js (Facebook version — AI-only product selection + female Thai sales specialist, 20-turn memory)
  ...
  try {
    ...
    for (const entry of body.entry) {
      ...
      for (const ev of msgs) {
        ...
        pushFragment(psid, text, async (frags) => {
...
# chatMemory.js (increase memory to last 20 turns, same Redis shape)
...
export async function setContext(psid, messages) {
  const trimmed = messages.slice(-20);          // keep last 20 turns (match LINE)
...
# package.json (no change required, but ensure Node ≥18 for global fetch)

```json
{
  "name": "my-shop-prices",
  "version": "1.0.0",
  "description": "Facebook Messenger bot connected to OpenRouter",
  "main": "index.js",
  "type": "module",
...
* Behaviour, prompts, CSV parsing, one-turn size/bundle carry, and 15s semantic reassembly now mirror the LINE implementation.
* Set env vars: `FACEBOOK_PAGE_ACCESS_TOKEN`, `FACEBOOK_VERIFY_TOKEN`, `OPENROUTER_API_KEY`, `MODEL` (optional), `REDIS_URL`, `CHAT_TTL_SECONDS` (optional).
* products.csv supports columns: name, price, unit, aliases, tags, specification (ขนาด), pcs\_per\_bundle.
